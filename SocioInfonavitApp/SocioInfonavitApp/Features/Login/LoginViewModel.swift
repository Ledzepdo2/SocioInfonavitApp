...
final class LoginViewModel: ObservableObject {
    ...
    func login(completion: @escaping (Bool) -> Void) {
        guard isFormValid else {
            AppErrorManager.shared.present(error: .validation(message: "Por favor llena todos los campos"))
            return
        }
        
        guard validateUserOrEmail(email) else {
            AppErrorManager.shared.present(error: .validation(message: "Debes ingresar un número de usuario válido o un email"))
            return
        }
        
        guard validatePassword(password) else {
            AppErrorManager.shared.present(error: .validation(message: "La contraseña debe tener 8+ caracteres, mayúscula, minúscula, número y caracter especial"))
            return
        }
